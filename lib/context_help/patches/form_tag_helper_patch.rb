  ...
  module Helpers
    module FormTagHelper
      def form_tag_with_context_help_form_tag(url_for_options = {}, options = {}, *parameters_for_url, &block)
        help_options = ContextHelp::Base.merge_options({:context_help => {:path => {:tag => :form, :tag_options => options}}}, options)
      ...
      def label_tag_with_context_help_label_tag(name, text = nil, options = {})
        help_options = ContextHelp::Base.merge_options({:context_help => {:path => {:tag => :label, :tag_options => options}}}, options)
